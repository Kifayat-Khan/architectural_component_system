...
# app.py — PART 1 of 3
# =========================
# Local, open source only. No external APIs.

# Deps (install once):
# pip install -U streamlit pillow numpy scikit-image matplotlib reportlab \
#   opencv-python-headless transformers accelerate sentencepiece safetensors torch torchvision
...
# ---- model ids
LLM_MODEL_ID  = "google/flan-t5-base"              # text2text for Q&A grounding
CLIP_MODEL_ID = "openai/clip-vit-base-patch32"     # image encoder
TRANS_MODEL_ID = "Helsinki-NLP/opus-mt-en-zh"      # EN -> ZH translator
...
def get_clip(device: str):
    if _CLIP["model"] is None:
        _CLIP["model"] = CLIPModel.from_pretrained(CLIP_MODEL_ID).to(device).eval()
        _CLIP["proc"]  = CLIPProcessor.from_pretrained(CLIP_MODEL_ID)
        _CLIP["device"] = device
...
if page == "Database Manager":
    ...
    st.info("Add building entries for retrieval and grounding.")
    name = st.text_input("Building Name")
    location = st.text_input("Location (City, Area)")
    era = st.text_input("Era / Period")
    ...
    if image_file and st.button("Add to Database"):
        if not name:
    ...
    st.stop()  # do not run analysis on this page
...
# ---------- ORB retrieval with geometric check ----------
...
def clip_aesthetic_score(pil_img):
    clip_model, clip_proc, _ = get_clip(device)
...
# ---------- LLMs: FLAN-T5 for grounded Q&A + EN->ZH translator ----------
...
def load_llm_and_clip():
    ...
    # warm index once
    build_index(str(CARDS_PATH), str(IDX_PATH), dev)
    return dev, t5_tok, t5, zh_tok, zh_mt

device, t5_tok, t5_model, zh_tok, zh_model = load_llm_and_clip()
...
def translate_en_to_zh(paragraphs_en: List[str]) -> List[str]:
    ...
    for para in paragraphs_en:
        ...
        gen = zh_model.generate(
            **enc, max_new_tokens=512, num_beams=4, length_penalty=1.0, no_repeat_ngram_size=3
        )
        ...
        outs.append(zh)
    return outs
# ---------- VLM: Qwen2-VL-2B-Instruct primary, moondream2 fallback ----------
_VLM = {"model": None, "proc": None, "tokenizer": None, "dtype": None, "device": None, "name": None}
...
def _downscale(pil: Image.Image, max_side: int = 512) -> Image.Image:
    w, h = pil.size
    if max(w, h) <= max_side:
        return pil
    pil2 = pil.copy()
    pil2.thumbnail((max_side, max_side), Image.LANCZOS)
    return pil2

def load_vlm(model_id_qwen: str = "Qwen/Qwen2-VL-2B-Instruct", dtype_pref="auto"):
    """
    Prefer Qwen2-VL on GPU/MPS. On CPU, default to moondream2 for speed.
    """
    if _VLM["model"] is not None:
        return _VLM["model"], _VLM["proc"], _VLM["tokenizer"], _VLM["device"], _VLM["dtype"], _VLM["name"]

    dev = _best_device()
    if dtype_pref == "auto":
        dtype = torch.float16 if dev in ("cuda","mps") else torch.float32
    else:
        dtype = getattr(torch, dtype_pref)

    prefer_qwen = (dev in ("cuda","mps")) or (os.getenv("FORCE_QWEN_CPU", "0") == "1")

    if prefer_qwen:
        try:
            proc = AutoProcessor.from_pretrained(model_id_qwen, trust_remote_code=True)
            model = AutoModelForCausalLM.from_pretrained(
                model_id_qwen, torch_dtype=dtype, trust_remote_code=True, device_map={"": dev}
            ).eval()
            _VLM.update({"model": model, "proc": proc, "tokenizer": None, "device": dev, "dtype": dtype, "name": model_id_qwen})
            return model, proc, None, dev, dtype, model_id_qwen
        except Exception:
            pass  # fall through to moondream2

    # Fallback: moondream2 (fast on CPU)
    fb = "vikhyatk/moondream2"
    proc = None
    model = AutoModelForCausalLM.from_pretrained(
        fb, torch_dtype=dtype, trust_remote_code=True, device_map={"": dev}
    ).eval()
    tokenizer = AutoTokenizer.from_pretrained(fb, trust_remote_code=True)
    _VLM.update({"model": model, "proc": proc, "tokenizer": tokenizer, "device": dev, "dtype": dtype, "name": fb})
    return model, proc, tokenizer, dev, dtype, fb

def _vlm_chat(pil_image: Image.Image, prompt: str, max_new_tokens=180) -> str:
    """
    Unifies calling across Qwen2-VL and moondream2, with downscaling and safe fallbacks.
    """
    model, proc, tokenizer, dev, _, name = load_vlm()
    img_small = _downscale(pil_image, 512)

    # --- Qwen2-VL path ---
    if "Qwen" in (name or ""):
        messages = [
            {"role": "system", "content": "You are a precise architecture and data visualization explainer."},
            {"role": "user", "content": [{"type": "image", "image": img_small}, {"type": "text", "text": prompt}]},
        ]
        inputs = proc.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt")
        if hasattr(proc, "process_images"):
            image_inputs = proc.process_images([img_small], return_tensors="pt")
            if isinstance(inputs, dict):
                for k, v in inputs.items(): inputs[k] = v.to(dev)
            else:
                inputs = {"input_ids": inputs.to(dev)}
            for k, v in image_inputs.items(): inputs[k] = v.to(dev)
        else:
            if hasattr(inputs, "to"): inputs = {"input_ids": inputs.to(dev)}
            else:
                inputs = {k: v.to(dev) for k, v in inputs.items()}
        with torch.inference_mode():
            gen = model.generate(
                **inputs, max_new_tokens=max_new_tokens, temperature=0.0, do_sample=False, use_cache=True
            )
        text = proc.batch_decode(gen, skip_special_tokens=True)[0]
        return text.split("assistant\n")[-1].strip()

    # --- moondream2 path ---
    if "moondream" in (name or "").lower():
        with torch.inference_mode():
            image_emb = model.encode_image(img_small)
            out = model.answer_question(image_emb, prompt, tokenizer)
            return (out or "").strip()

    # --- generic ultra-fallback (text only) ---
    decoder = proc if proc is not None else t5_tok
    inputs = decoder(prompt, return_tensors="pt").to(dev)
    with torch.inference_mode():
        gen = model.generate(**inputs, max_new_tokens=max_new_tokens, temperature=0.0, do_sample=False, use_cache=True)
    return decoder.batch_decode(gen, skip_special_tokens=True)[0].strip()

def vlm_facade_narrative(pil_image, lang="en"):
    prompt = (
        "Write a detailed architectural narrative about this building facade in three paragraphs. "
        "Paragraph 1: describe the composition, form, and massing — note how the structure occupies its space. "
        "Paragraph 2: explain visual organization — symmetry/asymmetry, rhythm, repetition, proportions, and materials visible from the street. "
        "Paragraph 3: interpret the atmosphere and aesthetic impression — how light, texture, and detail affect perception. "
        "Avoid meta or evaluative text (no phrases like 'this image shows'). "
        "Do not describe interiors or surroundings. Keep language descriptive and professional, in English only."
    )

    en = _vlm_chat(pil_image, prompt, max_new_tokens=380)

    # Guarantee at least 3 paragraphs (split if model gives one block)
    parts = [p.strip() for p in en.split("\n") if p.strip()]
    if len(parts) < 3:
        text = en.strip()
        # simple heuristic: split every ~3 sentences
        sentences = re.split(r'(?<=[.!?])\s+', text)
        chunks = [" ".join(sentences[i:i+3]) for i in range(0, len(sentences), 3)]
        en = "\n\n".join(chunks[:3])

    if lang == "zh":
        return translate_en_to_zh([en])[0]
    return en


def chart_explainer_text_only(metrics: Dict[str, Any], lang="en") -> str:
    """
    Fast, deterministic backup: turn your computed metrics into 5-7 concise bullets.
    """
    def lvl(x, lo, hi):
    ...
    bullets_en = [
        f"- Left–right symmetry is {lvl(vs, 0.4, 0.85)}; overall order feels {'balanced' if vs>=0.65 else 'relaxed'}.",
        f"- Rotational symmetry reads {lvl(rs, 0.35, 0.8)}, affecting how centered the mass feels.",
        f"- Proportion (H/W≈{prop:.2f}) suggests a {'slender vertical' if prop>1.9 else ('grounded horizontal' if prop<1.1 else 'balanced')} stance.",
        ...
        f"- Detail across scales is {lvl(fr, 1.2, 1.6)}, steering the surface toward {'ornate' if fr>=1.55 else ('plain' if fr<=1.25 else 'measured')} character.",
        "- Tip: capture a front-on view with full height to strengthen symmetry and rhythm cues."
    ]
    if lang == "zh":
...
def vlm_chart_explanation(pil_chart, lang="en", fallback_metrics: Optional[Dict[str,Any]] = None) -> str:
    """
    Try VLM; if empty/too short, return text-only fallback using metrics.
    """
    prompt = (
        "You see a chart or plot. Provide 5-7 concise bullet points explaining what it implies. "
        "Discuss relative levels, balance, trends, repetition, and dominant peaks or gaps. "
        "Do not quote exact numbers. Each bullet must start with '- '. English only."
    )
    try:
        en = _vlm_chat(pil_chart, prompt, max_new_tokens=160)
    except Exception:
        en = ""

    if not en or len(en.strip()) < 10:
        return chart_explainer_text_only(fallback_metrics or {}, lang=lang)
...
def guide_answer_from_info(question: str, info_text: str, LANG: str) -> str:
    ...
    inst_en = (
        ...
        "Answer only using the Information. If the answer is not in the Information, reply exactly: NOTFOUND.\n\n"
        f"Information:\n{info_text}\n\nQuestion: {question}\nAnswer:"
    )
    inst_zh = (
        "你是历史建筑导览员。只根据以下信息回答。"
    ...
    out = t5_model.generate(
        **enc, max_new_tokens=500, min_new_tokens=20, num_beams=5, do_sample=False,
        length_penalty=1.05, no_repeat_ngram_size=3, early_stopping=True
    )
...
def show_interior_gallery(card: dict):
    ...
    for i, item in enumerate(interiors):
        path = item.get("image"); cap = item.get("caption", "Interior view")
    ...
    if uploaded:
        for up in uploaded:
            ...
            with st.spinner("Computing heatmap..."):
                # simple combined edge and saliency map
                im = np.array(pil)
                if im.ndim == 2:
                ...
                heat = Image.alpha_composite(base, heat_img).convert("RGB")
            ...
            try:
                verified_card, combined, dbg = retrieve_verified(
                    ...
                    base_threshold=0.34, margin=0.05, inlier_floor=0.06, alpha=0.85
                )
                if verified_card:
                    match_info = (
                        f"DB match: {verified_card.get('name','?')} "
                        f"(combined={dbg.get('combined',0):.3f}, s1={dbg.get('s1',0):.3f}, inliers={dbg.get('inliers',0):.3f})"
                    )
                else:
                    match_info = f"No reliable DB match ({dbg.get('reason','?')}, s1={dbg.get('s1',0):.3f})"
            ...
            # VLM narrative
            with st.spinner("Generating facade narrative (VLM)..."):
                story_vlm = vlm_facade_narrative(pil, lang=LANG)
            ...
            story = (story_vlm or "").strip() + extra

            # Ask box with grounded info
            st.markdown("### Ask / 问")
            ...
            if verified_card and verified_card.get("interiors"):
                show_interior_gallery(verified_card)
            ...
            with st.spinner("Explaining the chart..."):
                chart_explanation = vlm_chart_explanation(viz_img, lang=LANG, fallback_metrics=metrics_dict)
